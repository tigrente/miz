...
miz.directive("dashItemTeamSummary", function () {
    return {
        restrict: 'E',
        templateUrl: 'client/dashboard/components/dash-item-team-summary/dash-item-team-summary.ng.html',
        controllerAs: 'db_ts',
        bindToController: true,
        scope: {
            focusEngagementId: "="          // String - Sets a parent variable to the selected partner Id
        },
        transclude: true,
        controller: function ($scope, $reactive) {
                    ...
                    engagementCollection: () => {
                        ...
                        let selector = {
                            $and: [
                                ...
                                {
                                    $or: [
                                            {'bdOwnerLabel': {'$regex': '.*' + this.filter_search || '' + '.*', '$options': 'i'}},
                                            {'contractingPartnersLabel': {'$regex': '.*' + this.filter_search || '' + '.*', '$options': 'i'}},
                                            {'cooperationResourcesLabel': {'$regex': '.*' + this.filter_search || '' + '.*', '$options': 'i'}},
            ...
            this.viewEngagementDetail = function (engagementId) {
                window.open("/engagements/" + engagementId);
            };
...
// The below jquery adjusts the height of the table body to accomodate the screen height

            $(window).resize(function () {
                var bodyheight = $('body').height();
                bodyheight = bodyheight - 200;
                $('#db-team-summary-table-body').css('min-height', bodyheight + 'px');
            }).resize();

            $(window).resize(function () {
                var bodyheight = $('body').height();
                bodyheight = bodyheight - 200;
                $('#db-team-summary-table-body').css('max-height', bodyheight + 'px');
            }).resize();

            $(window).resize(function () {
                let bodywidth = $('body').width();
                bodywidth = bodywidth - 90;
                $('#db-team-summary-table-body').css('width', bodywidth + 'px');
            }).resize();
